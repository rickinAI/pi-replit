...
async function openPosition(params) {
  ...
  if (mode === "SHADOW") {
    ...
    const portfolio2 = await getPortfolioValue();
    ...
    positions2.push(shadowPosition);
    await savePositions(positions2);
...
var lastDedupeReset = "";
async function loadPersistedAlertDedup() {
  try {
    const { getPool: getPool2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    const pool2 = getPool2();
    const res = await pool2.query(`SELECT value FROM app_config WHERE key = 'alerted_calendar_events'`);
    if (res.rows.length > 0 && res.rows[0].value) {
...
async function persistAlertDedup() {
  try {
    const { getPool: getPool2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    const pool2 = getPool2();
...
var ACTIONABLE_EMAIL_CATEGORIES = /* @__PURE__ */ new Set(["Travel", "Financial", "Documents", "Calendar"]);
...
async function startAlertSystem(broadcast, saveBrief) {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      for (const [jobId, updates] of Object.entries(costOptJobs)) {
        const j = config2.jobs.find((x) => x.id === jobId);
        if (!j) continue;
        ...
        if (updates.enabled !== void 0 && j.enabled !== updates.enabled) {
...
async function buildWealthEnginesDashboardData() {
  ...
  let eoyTarget = 5e4;
  try {
    const eoyRes = await pool2.query(`SELECT value FROM app_config WHERE key = 'wealth_engines_eoy_target'`);
...
app.post("/api/wealth-engine/portfolio", async (req, res) => {
  ...
  try {
    ...
    const pool2 = getPool();
    const oldPortfolio = await getPortfolioValue();
    ...
    const newPortfolio = await getPortfolioValue();
    const newPeak = await getPeakPortfolioValue();
    weDashboardCache = null;
    if (typeof value === "number" && value !== oldPortfolio) {
      const rc = await getRiskConfig();
      const multiplier = oldPortfolio > 0 ? (value / oldPortfolio).toFixed(0) + "x" : "";
      const modeRes = await pool2.query(`SELECT value FROM app_config WHERE key = 'wealth_engines_mode'`);
      ...
      const fmtD = (n) => n >= 1e3 ? `$${(n / 1e3).toFixed(n % 1e3 === 0 ? 0 : 1)}K` : `$${n.toFixed(2)}`;
      const lines = [
        `\u{1F4B0} *Portfolio Updated*`,
        ``,
...
async function runStartupRecovery() {
  ...
  try {
    ...
    if (portfolioRes.rows.length > 0 && (portfolioRes.rows[0].value === 50 || portfolioRes.rows[0].value === 1e3)) {
      await migPool.query(
        `UPDATE app_config SET value = $1, updated_at = $2 WHERE key = 'wealth_engines_portfolio_value'`,
        [JSON.stringify(1e4), Date.now()]
      );
    ...
    if (peakRes.rows.length > 0 && (peakRes.rows[0].value === 50 || peakRes.rows[0].value === 1e3)) {
      await migPool.query(
    ...
    try {
      await new Promise((resolve, reject) => {
        ...
        server.listen({ port: PORT, host: "0.0.0.0", exclusive: false }, async () => {
          ...
          startJobSystem(
            async (agentId, task, onProgress, toolSubset) => {
              let agentTools = [
                ...buildKnowledgeBaseTools(),
                ...cachedStaticTools
              ];
