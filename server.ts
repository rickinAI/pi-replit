...
async function authMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  if (darknodeApiKey) {
    const keyFromQuery = req.query.apikey as string | undefined;
    if (keyFromQuery === darknodeApiKey) { (req as any).user = "rickin"; next(); return; }
  }
...
async function buildPnlDashboardData(): Promise<any> {
  ...
  return {
    ...
    all_trades: allTradesCombined.map((t: any) => ({
      ...
      is_copy_trade: t.is_copy_trade || false,
      source_wallet: t.source_wallet || null,
      category: t.category || t.exposure_bucket || null,
    })),
          ...
          (async () => {
            try {
              ...
              await clobStream.start(async (tokenId: string, price: number) => {
                try {
                  const tradeId = clobStream.getTradeIdForToken(tokenId);
                  if (!tradeId) return;

                  const oversightMod = await import("./src/oversight.js");
                  const trades = await oversightMod.getShadowTrades("open");
                  ...
                  if (trade.target_price != null) {
                    const targetHit = isYes ? price >= trade.target_price : price <= trade.target_price;
                    if (targetHit) {
              ...
              console.log(`[boot] CLOB price stream: ${subCount} tokens subscribed (${wsStatus})`);
            } catch (err) {
              console.warn("[boot] CLOB stream init failed (shadow prices will use scheduled refresh):", err instanceof Error ? err.message : err);
            }
          })();
