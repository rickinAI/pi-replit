...
- **src/gmail.ts** — Full Gmail integration via custom Google OAuth (12 tools: list, read, search, get_attachment with PDF extraction, thread, send, reply with threading headers, draft, archive, label, mark_read, trash). Tokens stored in PostgreSQL (oauth_tokens table). OAuth scopes: gmail.modify, calendar, drive, spreadsheets, documents, presentations, youtube.readonly. Also exports `getAccessToken()` for gws CLI and YouTube API
