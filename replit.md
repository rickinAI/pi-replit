...
- **Autonomous WE Control API** — Full REST API for DarkNode self-management. All endpoints require DARKNODE_API_KEY auth (apikey param or Bearer header). `GET /api/controls` returns unified system state (paused, killSwitch, mode, circuitBreaker, portfolio, riskConfig). `PUT /api/controls` updates system state with Telegram alerts. `POST /api/telegram/send` sends arbitrary Telegram messages (max 4000 chars, Markdown/HTML). **Whale Registry CRUD**: `GET /api/whale-registry` lists wallets+blacklist, `POST /api/whale-registry` adds wallet (runs decode gate by default, `skipDecode: true` for manual), `PUT /api/whale-registry/:address` updates fields (alias, niche, enabled, strategy, maxCopyPrice, minTradeSize, status), `DELETE /api/whale-registry/:address` removes wallet (optional `blacklist: true`). `GET /api/cost-summary` for API spend monitoring. All writes are WE_CONTROL_USERS gated (rickin, darknode)
