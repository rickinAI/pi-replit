...
- **src/bankr.ts** — BANKR unified execution engine. Pre-trade risk checks (kill switch, pause, leverage, margin, exposure buckets, correlation). Position sizing: 2% max risk/trade. Shadow mode support. FIFO tax lots with wash sale detection. Position CRUD. Circuit breaker: -15% rolling 7-day drawdown. Independent position monitor (5-min setInterval, NOT in agent job queue): trailing stop 5.5x ATR, RSI exit 69/31, kill switch check. Monitor heartbeat stored in `bankr_monitor_last_tick` DB key. Tools: bankr_risk_check, bankr_open_position, bankr_close_position, bankr_positions, bankr_trade_history, bankr_tax_summary
