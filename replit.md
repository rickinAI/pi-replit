...
- **src/copy-trading.ts** — Autonomous Polymarket copy trading engine. Tracks 5-10 whale wallets, detects new positions via snapshot diffing (every 4h polling), mirrors trades through BANKR in SHADOW mode. Wallet registry CRUD (app_config-backed, key `copy_trade_wallets`). Position snapshots stored in `copy_trade_snapshots` DB key — diffing detects entries AND exits. Copy trade evaluation filters: wallet win rate ≥65%, odds 15¢-85¢, volume >$10k, >24h to resolution, max 3 concurrent copy positions, $100 portfolio drawdown hard stop. Fixed $50/trade sizing. Whale exit mirroring: closes copy position when source wallet exits. Market resolution detection with P&L recording. 6h-to-close proximity alerts via Telegram. Per-wallet performance tracking (wins, losses, P&L, win rate) in `copy_trade_performance` DB key. `runCopyTradeScan()` entry point called directly from scheduled jobs (no agent overhead). Telegram commands: /wallets (list tracked wallets), /copytrades (active copy positions). API: GET/POST/DELETE `/api/copy-trading/wallets`, GET `/api/copy-trading/performance`. Scheduled job: `copy-trade-scan` every 4h in `scheduled-jobs.ts`
