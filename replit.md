...
- **src/scheduled-jobs.ts** — Scheduled agent jobs system. Runs agents autonomously on configurable schedules. 31 jobs (16 enabled — 10 WE jobs paused for cost savings ~$510/month). Config stored in `app_config` key='scheduled_jobs'. 60s check loop with run-key dedup (interval jobs use timestamp-based dedup). Schedule types: `daily`, `weekly`, `interval` (with `intervalMinutes`). Status tracking: `success`, `partial` (timedOut or "PARTIAL" in response), `error`. Writes `Scheduled Reports/job-status.json` to vault after each job. Per-job `toolSubset` field restricts tools sent to Anthropic API (reduces input tokens for lightweight scans). API: GET/POST/PUT/DELETE `/api/scheduled-jobs`, POST `/api/scheduled-jobs/:id/trigger`, GET `/api/scheduled-jobs/history`, GET `/api/kb/read?path=...`
...
- **src/signal-sources.ts** — Signal source aggregator for Wealth Engines SCOUT. 10 functions: Fear & Greed Index (alternative.me), Binance multi-TF signals (1h/4h/1d RSI/MACD/SMA/volume from Binance US spot), watchlist scanner (batch signal analysis), liquidations (Coinglass with Binance US fallback), sentiment (LunarCrush + CryptoPanic aggregation), DefiLlama TVL (protocols + chain breakdown), DefiLlama yields (top pools by TVL), funding rates (estimated from spot data), open interest (estimated from spot data), enhanced CoinGecko (trending + dominance + DeFi). Bounded LRU cache (200 entries, 5-min TTL). Estimated data sources labeled with `source: "binance_us_spot_estimated"`. Tools registered in `buildSignalSourceTools()` in server.ts
...
- **src/dexscreener.ts** — DexScreener API integration for on-chain DEX data. Free API, no key required. Functions: searchPairs (by name/symbol/address), getTokenPairs (by chain+address), getTrendingPairs (boosted tokens with full pair data), getTopBoostedTokens (promoted tokens), getLatestTokenProfiles (new listings). Returns standardized DexPair objects with price, 5m/1h/6h/24h changes, volume, liquidity, FDV, chain, DEX, pair age. Tools: dex_search, dex_token_pairs, dex_trending, dex_boosted_tokens, dex_new_tokens. Primary discovery source for Scout — surfaces on-chain momentum and new launches before centralized exchanges
- **src/technical-signals.ts** — Technical analysis signals module for Wealth Engines. Returns continuous confidence scores (0.0-1.0), not booleans. 6 active signals: EMA crossover, RSI, momentum, very short momentum, MACD, BB width compression. Ensemble scoring (weighted average of active signals). Market regime classifier (TRENDING/RANGING/VOLATILE). ATR calculation with configurable stop multiplier (default 5.5x). DB-backed parameter loading: `loadCryptoSignalParams()` reads from `crypto_signal_parameters` app_config key (5-min cache), falls back to hardcoded defaults. `invalidateCryptoParamsCache()` for immediate reload after autoresearch updates. Tool: technical_analysis
- **src/autoresearch.ts** — Autoresearch strategy optimization engine. Two domains (crypto + polymarket) with shared mutation/scoring infrastructure. Crypto: 17-parameter space (EMA, RSI, MACD, BB, momentum, ATR, vote threshold, cooldown), bar-by-bar backtest on CoinGecko OHLCV data, Nunchi scoring (Sharpe x sqrt(trade_count_factor) - drawdown_penalty - turnover_penalty). Polymarket: 15-parameter space (wallet score, consensus, odds, sizing tiers, category weights, exit/stale thresholds), historical outcome replay backtest, ROI+win_rate scoring. Candidate generation: mutate 1-3 params within valid ranges, max 30% drift per run. Safety: min 0.5% improvement to adopt, max 30% parameter drift, 5-deep rollback history per domain. DB keys: crypto_signal_parameters, polymarket_scout_parameters, autoresearch_experiment_log, autoresearch_crypto_history, autoresearch_polymarket_history. Tools: autoresearch_run, autoresearch_status, autoresearch_rollback, autoresearch_experiment_log, autoresearch_params. Scheduled: weekly Sunday 3am. Telegram: /research [crypto|polymarket|rollback|status]
...
- **src/bnkr.ts** — BNKR API wrapper for Avantis on-chain perps on Base. Crypto: openCryptoPosition, closeCryptoPosition, getCryptoPositions, getCryptoPositionPnL. Polymarket: openPolymarketPosition, closePolymarketPosition, getPolymarketPositions, getPolymarketPositionPnL. Auth: BNKR_API_KEY + BNKR_WALLET_ADDRESS. Shadow mode when unconfigured (logs but doesn't execute). 15s timeout per request
- **src/coinbase-wallet.ts** — Coinbase Wallet API for spot operations (manual funding-only rail, not used for trade execution). HMAC-SHA256 signed requests. buySpot, sellSpot, getBalances. Auth: COINBASE_API_KEY + COINBASE_API_SECRET. Shadow mode when unconfigured. Account lookup by currency
- **src/bankr.ts** — BANKR unified execution engine. BNKR-only venue (Coinbase removed from execution path). $10,000 starting capital. **DB-backed risk config** (`wealth_engine_config` key): all risk parameters are dynamic and editable via Control Panel or API. RiskConfig interface: max_leverage, risk_per_trade_pct, max_positions, exposure_cap_pct, correlation_limit, circuit_breaker_7d_pct, circuit_breaker_drawdown_pct, notification_mode. Defaults: 5x leverage, 5% risk/trade, 3 max positions, 60% exposure cap, 1/bucket correlation, -15% 7d breaker, -25% drawdown breaker. getRiskConfig()/setRiskConfig() with validation. Tiered risk framework: autonomous (≤20% capital, confidence ≥3.5), dead_zone (20-30%, flag in Telegram), human_required (>30%, 3+ consecutive losses, drawdown, first trade on asset). Shadow mode support. FIFO tax lots with dynamic holding period + wash sale detection. Source tracking (crypto_scout/polymarket_scout/manual) on Position and TradeRecord. Peak portfolio value tracked automatically. Consecutive loss counter resets on win. **Signal Quality Feedback Loop** (`signal_quality_scores` DB key): tracks per-source (crypto_scout, polymarket_scout), per-asset-class win/loss counts, rolling win rate with 30-day time decay, avg P&L, profit factor (gross profit / gross loss), and per-asset breakdown (wins/losses/win_rate/avg_pnl per individual asset). Updated automatically on terminal trade/shadow close only (stop_hit, target_hit, expired, rsi_exit, time_exit, manual — NOT replaced_by_newer_thesis). Trade ID deduplication prevents double-counting. getSignalQuality()/updateSignalQuality()/getSignalQualityModifier() functions. Modifier requires min 8 samples: boost when win rate >55% AND profit factor >1.2, penalty when win rate <40% OR profit factor <0.5. All metrics (profit_factor, asset_breakdown) recomputed on decay pruning. SCOUT agents query signal_quality tool before thesis generation to adjust confidence. Independent position monitor (2-min setInterval, NOT in agent job queue): trailing stop 5.5x ATR, RSI exit 69/31, 72h crypto time exit, polymarket odds target, 10-point stop loss, whale consensus flip, 30d underwater exit, resolution proximity (<4h), kill switch check. Monitor heartbeat stored in `bankr_monitor_last_tick` DB key. Tools: bankr_risk_check, bankr_open_position, bankr_close_position, bankr_positions, bankr_trade_history, bankr_tax_summary, signal_quality
- **data/pages/wealth-engines-pnl.html** — P&L & Tax Dashboard at rickin.live/pages/wealth-engines-pnl. Dedicated page with equity curve chart (canvas-based, no CDN), drawdown visualization, trade markers (green wins, red losses with hover tooltips), tabbed layout (Equity Curve / Tax Summary / Tax Lots). Tax summary: YTD realized P&L, short-term vs long-term gains separation, quarterly breakdown grid, wash sale flags, estimated federal (24% ST, 15% LT) and NY State (6.85%) taxes. Tax lots: FIFO lot listing with cost basis, proceeds, gain/loss, holding period badges, wash sale indicators. SSR data injection for fast load. 60-second auto-refresh. Summary card on main Wealth Engines dashboard links to this page. API: GET /api/wealth-engines/pnl-data (30s cache)
- **data/pages/wealth-engine-controls.html** — Wealth Engine Control Panel at rickin.live/pages/wealth-engine-controls. Password-protected, dark-themed, mobile-first. Sections: Risk Parameters (sliders/dropdowns for all config), Execution Controls (mode toggle, pause/resume, kill switch), Agent Schedules (enable/disable 10 WE jobs, adjust interval timing), Portfolio (view/set portfolio & peak values), Notification Preferences, System Health (uptime, monitor tick, shadow trade stats). Auto-refreshes every 30s. API: GET/POST /api/wealth-engine/config, POST /api/wealth-engine/portfolio, POST /api/wealth-engine/mode, POST /api/wealth-engine/jobs/:id
- **src/oversight.ts** — Wealth Engines Oversight Agent core logic. Operational risk monitoring, performance review, cross-domain exposure detection, improvement capture, and shadow trading. Health checker evaluates 9 subsystems (SCOUT/BANKR/PM freshness, monitor heartbeat, kill switch, pause state, circuit breaker, data freshness, job failures). Performance review: win rate, Sharpe ratio, avg P&L, slippage, source attribution (crypto_scout vs polymarket_scout), thesis conversion rate. Cross-domain exposure detector: correlates crypto positions with Polymarket theses via keyword matching (BTC↔bitcoin, ETH↔ethereum, etc.), flags combined exposure %. Improvement request lifecycle: open→accepted→resolved/dismissed with dedup (24h window). Shadow trading: hypothetical trades tracked in parallel with P&L calculation. Auto-prunes data >30 days. DB keys: oversight_health_reports, oversight_improvement_queue, oversight_shadow_trades, oversight_last_health_check. Tools: oversight_health_check, oversight_performance_review, oversight_cross_domain_exposure, oversight_improvement_queue, oversight_capture_improvement, oversight_update_improvement, oversight_shadow_open, oversight_shadow_close, oversight_shadow_trades, oversight_shadow_performance, oversight_summary
- **src/telegram.ts** — Telegram Command Center for Wealth Engines mobile control. Direct Bot API via fetch (no library). Webhook mode with long-polling fallback. Two bots: DarkNode (@Darknode_trading_bot) for trading events + Mission Control (@MissionControl_alerts_bot) for personal alerts + status digests. Commands: /status (enhanced: F&G, shadow P&L, PM scout timestamp, notify mode), /portfolio, /intel, /pause, /resume, /scout, /polymarket, /trades [n], /kill, /risk, /oversight, /shadow, /notify [smart|immediate], /research [crypto|polymarket|rollback|status], /tax, /reset [capital] (full portfolio reset: clears positions, trades, shadow trades, signal quality scores, kill switch, pause — default $10K), /public on|off, /alerts, /help. Job completion notifications: WE jobs (scout/PM/BANKR/oversight) send Telegram summaries on completion with smart dedup (micro-scans only notify on material changes). Job failure alerts with error snippets. Shadow trade open/close notifications with P&L. DarkNode summary at fixed ET times (9am/12pm/3pm/6pm/9pm) via scheduled-jobs (portfolio value, today's closed trades with % return, W/L stats, Fear & Greed, alerts). Shadow trade close notifications enriched with % return, hold duration, running totals. Live trade close notifications include % return, hold duration, labeled reason. Inline keyboard trade approval flow (approve/skip/hold with 30m timeout). Dead man's switches: SCOUT no run >6h, BANKR agent no run >8h, BANKR monitor no tick >30min. Notification mode config stored in app_config ('we_notification_mode': smart|immediate|digest). Env vars: TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, TELEGRAM_ALERTS_BOT_TOKEN, TELEGRAM_ALERTS_CHAT_ID
