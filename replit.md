...
- **src/telegram.ts** — Telegram Command Center with single-bot, 12-channel architecture. Direct Bot API via fetch (no library). Webhook mode with long-polling fallback. **Single bot** (`TG_BOT_TOKEN`, @DarkNode_control_bot) routes to 12 channels: direct (default), retuned, moodys, family, real-estate, ai-tech, bitcoin, markets, news, intel, trading, mission-control. Channel chat IDs stored as `TG_CHANNEL_*` env vars. **Core API**: `sendToChannel(channel, text, parseMode)` — unified routing function for all outbound messages. `sendAlertsBotMessage()` → routes to mission-control for backward compat. **Channel routing**: trade alerts/scout briefs/oversight → trading, dead man switches/circuit breakers/system alerts/briefs → mission-control, personal alerts → direct. `telegram_send` tool registered for all 14 agents (message + optional channel param). `POST /api/telegram/send` REST endpoint accepts `{message, channel?, parse_mode?}`. All alert/brief messages use HTML parse_mode. Commands: /status, /portfolio, /intel, /pause, /resume, /scout, /polymarket, /trades [n], /kill, /risk, /oversight, /shadow, /notify, /tax, /reset, /public, /alerts (shows channel config), /wallets, /copytrades, /walletstatus, /seedwallets, /goal, /help
