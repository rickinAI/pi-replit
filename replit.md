...
- **src/db.ts** — Shared PostgreSQL connection pool (single `pg.Pool`, max 10 connections). Creates 11 tables on init (`conversations`, `tasks`, `app_config`, `oauth_tokens`, `job_history`, `agent_activity`, `vault_inbox`, `email_pipeline`, `pages`, `personal_context`, `telegram_conversation_history`, `interview_sessions`) + enables pgvector extension. `interview_sessions` has partial unique index on `(chat_id) WHERE status IN ('active','paused')` to prevent concurrent sessions. `vault_embeddings` table created by vault-embeddings module. `pages` table stores HTML pages (slug PK, html TEXT, is_public BOOLEAN DEFAULT false, created_at, updated_at) — survives redeploys. Boot sync compares `data/pages/*.html` file modification times against DB `updated_at` — newer files on disk automatically push updates (prevents stale pages). All other modules import `getPool()` from here
...
- **src/polymarket-scout.ts** — Polymarket SCOUT intelligence + wallet discovery. Thesis generation from whale consensus. **Auto-discovery**: `seedWalletsFromTradeStream()` mines recent trades for high-volume wallets, builds profiles, scores, auto-adds ≥0.6. `runAnomalyScanner()` scans 30-min trade window for unknown large wallets (≥$2K), promotes quality candidates as observation_only. `scoreAndPromoteCandidate()` builds full profile from activity API then runs `decodeWallet()` strategy gate. `detectAndBlacklistMarketMakers()` behavioral fingerprint (mmScore≥3/4: tradeCount>100, buy/sell symmetry<0.15, markets>20, avgTrade<$500). **Wallet Decode** (`decodeWallet()`): one-time strategy classification on admission. Pulls last 50 trades, classifies dominant niche via `categorizeMarketTitle()`/`determineNiche()`, detects anti-patterns. Scraper detection: avg entry >$0.90 AND >50% of buys above $0.90. MM detection: reuses existing 4-signal heuristic. Sets per-wallet `maxCopyPrice` (sports 0.80, politics 0.92, crypto 0.75, weather/general 0.85) and `minTradeSize` (median*0.3, min $25). Rejected wallets auto-blacklisted + Telegram alert. Admitted wallets get Telegram notification with strategy/thresholds. `migrateWalletRegistries()` one-time migration from old `copy_trading_wallets` → unified registry. Thresholds: score≥0.6, ≥1 whale (tiered: HIGH/MEDIUM/LOW/SPECULATIVE), ≥$50K volume, 15-85% odds, ≥24h to resolution. Scheduled jobs: `anomaly-scanner` (30min), `seed-wallets` (weekly Sunday 9am). Boot sequence: migrate → init blacklist → auto-seed if registry<3
- **src/github.ts** — GitHub API module using `@replit/connectors-sdk` proxy pattern. Fresh ReplitConnectors instance created per call (no caching, ensures token freshness). Functions: `listRepos()`, `listIssues(owner, repo, state)`, `readIssue(owner, repo, number)`, `createIssue(owner, repo, title, body, labels)`, `commentOnIssue()`, `listPRs()`, `readPR()`, `readFile(owner, repo, path, ref)`, `searchCode(query, owner, repo)`, `searchIssues(query, owner, repo, state)`. All authenticated via Replit GitHub integration (OAuth auto-managed). Known repos: `rickinAI/pi-replit` (DarkNode home repo), `retune-app/retune-app` (Retuned). Tools registered: `github_list_repos`, `github_list_issues`, `github_read_issue`, `github_create_issue`, `github_comment_issue`, `github_list_prs`, `github_read_pr`, `github_read_file`, `github_search`. Available to main agent + deep-researcher, moodys, oversight, retuned-pm, darknode agents. **Note**: retuned-pm has READ-ONLY tools only (no github_create_issue, no github_comment_issue) — all code changes go through Replit Agent exclusively. **DarkNode** has full read+write GitHub access and vault-to-GitHub bridge: fix requests/specs saved to vault are offered as GitHub issues in rickinAI/pi-replit with labels (fix-request, spec, enhancement, bug)
- **src/clob-stream.ts** — Real-time Polymarket CLOB price streaming for shadow trades. WebSocket connection to `wss://ws-subscriptions-clob.polymarket.com/ws/market` streams live price changes for all open shadow positions. On each price tick: updates `current_price`, recalculates `hypothetical_pnl` and `pnl_pct`, checks stop/target triggers. `resolveTokenIds(conditionId)` maps condition IDs to YES/NO CLOB token IDs via Gamma API (24h cache). Auto-subscribes new shadow trades on open. Falls back to REST midpoint polling (`GET /midpoint?token_id=`) every 5 minutes if WebSocket disconnects. Exponential backoff reconnect (5s→60s). No SDK, no wallet, no auth — pure public read-only price feed. Boot: starts streaming if open shadow trades exist, otherwise activates on first new trade
- **src/bnkr.ts** — BNKR Agent API wrapper for Polymarket execution on Polygon. Base URL: `https://api.bankr.bot`. Auth: `X-API-Key` header. All trade execution uses prompt-based async flow: `POST /agent/prompt` (submit natural language trade instruction) → `GET /agent/job/{jobId}` (poll 2s intervals, up to 120s). `openPolymarketPosition()` constructs prompt like `bet $50 on YES for "market question" on polymarket`, parses tx_hash and odds from response. `closePolymarketPosition()` sends `sell all my shares in "market" on polymarket`. `bnkrAnalyze()` for general BNKR queries. `checkBalance()` dry-run asks `what is my USDC balance on polygon?` — runs at boot for connectivity validation. `testConnectivity()` hits `/health` endpoint. Shadow mode when API key not configured (logs but doesn't execute). 30s timeout per request
- **src/bankr.ts** — BANKR execution engine (Polymarket only via BNKR). $10,000 starting capital. **DB-backed risk config** (`wealth_engine_config` key): all risk parameters are dynamic and editable via Control Panel or API. RiskConfig interface: max_leverage, risk_per_trade_pct, max_positions, exposure_cap_pct, correlation_limit, circuit_breaker_7d_pct, circuit_breaker_drawdown_pct, notification_mode, min_confidence_for_execution, thesis_blacklist_hours. Defaults: 5x leverage, 5% risk/trade, 3 max positions, 60% exposure cap, 1/bucket correlation, -15% 7d breaker, -25% drawdown breaker, MEDIUM min confidence, 48h thesis blacklist. **Confidence Gate** (RA-2): `runPreExecutionChecks()` now accepts `thesis_id`, `confidence_level`, `whale_count` params. Confidence ranking: HIGH(4) > MEDIUM(3) > LOW(2) > SPECULATIVE(1). Trades below `min_confidence_for_execution` are rejected and auto-shadowed. Copy trading also gated via confidence check in scanning loop. **Thesis Blacklist** (RA-1): stop-loss closes (`pm_stop_loss`, `pm_stop_loss_no_thesis`) auto-invalidate the thesis + all same-market theses for `thesis_blacklist_hours` (default 48h). `closePosition()` triggers `invalidateThesis()` + Telegram BLACKLIST notification. `runPreExecutionChecks()` rejects trades on blacklisted theses. Tiered risk framework: autonomous (≤20% capital, confidence ≥3.5), dead_zone (20-30%, flag in Telegram), human_required (>30%, 3+ consecutive losses, drawdown, first trade on asset). Shadow mode support. FIFO tax lots with dynamic holding period. Source tracking (polymarket_scout/manual/copy_trade) on Position and TradeRecord. Copy trade support: `is_copy_trade` flag, `source_wallet` field, `copy_trade_size_usd` param; SHADOW mode uses fixed $50 sizing for copy trades; position monitor skips percentage stop-loss when `is_copy_trade === true`. **Signal Quality Feedback Loop** (`signal_quality_scores` DB key): tracks per-source win/loss counts, rolling win rate with 30-day time decay, avg P&L, profit factor. `getSignalQualityModifier()` returns modifier/winRate/sampleSize/profitFactor for a given source. `generateForm8949CSV()` exports tax lots as CSV. Independent position monitor (2-min setInterval): polymarket odds target, 10-point stop loss, whale consensus flip, 30d underwater exit, resolution proximity (<4h), kill switch check. Tools: bankr_risk_check, bankr_open_position, bankr_close_position, bankr_positions, bankr_trade_history, bankr_tax_summary, signal_quality
...
- **Autonomous WE Control API** — Full REST API for DarkNode self-management (11 control endpoints + whale CRUD + telegram). All endpoints gated by WE_CONTROL_USERS (rickin, darknode). `GET /api/controls` returns unified system state (paused, killSwitch, mode, circuitBreaker, portfolio, riskConfig, blacklistedTheses). `PUT /api/controls` updates system state with Telegram alerts on change. Supports: `min_confidence_for_execution` (HIGH/MEDIUM/LOW/SPECULATIVE), `thesis_blacklist_hours` (number), `clear_blacklist` (thesis_id string) for RA-1/RA-2 management. `POST /api/controls/reset-portfolio` full reset (capital, trades, positions, shadow, signal quality). `POST /api/controls/reset-circuit-breaker` resets peak/losses/unpause without clearing trade history. `POST /api/controls/close-position` closes single position by ID. `POST /api/controls/close-all-positions` closes all with partial-failure detection (207 Multi-Status). `POST /api/controls/thesis` manages thesis lifecycle (retire/invalidate/clear_blacklist). `GET /api/controls/theses` lists active + blacklisted theses. `GET /api/controls/shadow-trades` views shadow trades + performance. `POST /api/controls/shadow-trades/close` closes individual shadow trade. `PUT /api/controls/risk-config` updates risk params with strict validation (negative thresholds for circuit breakers, enum checks for confidence/notification_mode). `POST /api/telegram/send` sends arbitrary Telegram messages (max 4000 chars, Markdown/HTML). **Whale Registry CRUD**: `GET /api/whale-registry` lists wallets+blacklist, `POST /api/whale-registry` adds wallet (runs decode gate by default, `skipDecode === true` strict boolean for manual curation), `PUT /api/whale-registry/:address` updates fields (alias, niche, enabled, strategy, maxCopyPrice, minTradeSize, status); `status: "blacklisted"` evicts from watchlist + adds to blacklist in one call, `DELETE /api/whale-registry/:address` removes wallet (optional `blacklist: true`). `GET /api/cost-summary` for API spend monitoring. `POST /api/scheduled-jobs/:id/trigger` for on-demand job execution
...
- **src/telegram.ts** — Telegram Command Center with single-bot, 12-channel architecture. Direct Bot API via fetch (no library). Webhook mode with long-polling fallback. Deterministic webhook secret (SHA-256 of bot token — stable across dev/deploy). **Single bot** (`TG_BOT_TOKEN`, @DarkNode_control_bot) routes to 12 channels: direct (default), retuned, moodys, family, real-estate, ai-tech, bitcoin, markets, news, intel, trading, mission-control. Channel chat IDs stored as `TG_CHANNEL_*` env vars. **Core API**: `sendToChannel(channel, text, parseMode)` — unified routing function for all outbound messages. `sendToChannelWithKeyboard(channel, text, keyboard, parseMode)` — sends messages with inline action buttons. `sendAlertsBotMessage()` → routes to mission-control for backward compat. **Two-way chat**: Non-command messages from authorized users (Rickin whitelist by Telegram user ID) forwarded to DarkNode oversight agent, response sent back to originating chat via `replyToChat(chatId)` with HTML→plaintext fallback. **Personal context injection**: `About Me/Telegram Context.md` synced to `personal_context` DB table on boot + every 6h + manual via `POST /api/personal-context/sync`. Injected into every webhook prompt. **DB conversation history**: `telegram_conversation_history` table — last 20 messages per chat_id, auto-prunes at 100. Persists across restarts. **Response cleaning**: strips "DARKNODE RESPONSE" headers and "System status unchanged" footers via `cleanAgentResponse()`. Unauthorized users silently ignored. **Morning Pack** (`sendMorningPack()`): Consolidated 7 AM ET digest to #direct — aggregates AI news, BTC/Fear&Greed, active theses, news, intel from DB. **EOD Rollup** (`sendEODRollup()`): 9 PM ET daily summary to #direct — portfolio P&L, scout cycles, theses, shadow trades, health alerts, top story, outstanding items. **Inline buttons**: Trade alerts (shadow open/close, trade alerts) include inline keyboards: Dashboard URL, Pause WE, Retire Thesis, View P&L. Callback handlers: `pause_we` sets wealth_engines_paused, `retire_thesis:<id>` removes thesis from active list, `view_dashboard`/`view_pnl` opens URL. All callbacks gated by authorized user whitelist. **Channel routing**: trade alerts/scout briefs/oversight → trading, dead man switches/circuit breakers/system alerts/briefs → mission-control, personal alerts → direct. `telegram_send` tool registered for all 14 agents (message + optional channel param). `POST /api/telegram/send` REST endpoint accepts `{message, channel?, parse_mode?}`. All alert/brief messages use HTML parse_mode. Commands: /status, /portfolio, /intel, /pause, /resume, /scout, /polymarket, /trades [n], /kill, /risk, /oversight, /shadow, /notify, /tax, /reset, /public, /alerts (shows channel config), /wallets, /copytrades, /walletstatus, /seedwallets, /goal, /interview [topic], /help. **Interview Mode** (`/interview [topic]` or "interview me about X"): Structured personal knowledge capture via adaptive Q&A. Agent asks one question at a time, adapts follow-ups based on answers and vault context. Work-topic guard rejects professional topics. Pause/resume supported (state persisted in `interview_sessions` DB table). On completion: synthesizes structured vault note (themes, insights, decisions, action items, wikilinks to related notes), routes to best-fit vault folder (Health/, Family/, Areas/Spirituality/, Journal/, etc.), saves raw transcript to Conversations/, offers task extraction. Stale sessions auto-cleared >7 days on boot. Partial unique index prevents concurrent sessions per chat
