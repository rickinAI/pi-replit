...
- **src/db.ts** — Shared PostgreSQL connection pool (single `pg.Pool`, max 10 connections). Creates 11 tables on init (`conversations`, `tasks`, `app_config`, `oauth_tokens`, `job_history`, `agent_activity`, `vault_inbox`, `email_pipeline`, `pages`, `personal_context`, `telegram_conversation_history`) + enables pgvector extension. `vault_embeddings` table created by vault-embeddings module. `pages` table stores HTML pages (slug PK, html TEXT, is_public BOOLEAN DEFAULT false, created_at, updated_at) — survives redeploys. Boot sync compares `data/pages/*.html` file modification times against DB `updated_at` — newer files on disk automatically push updates (prevents stale pages). All other modules import `getPool()` from here
